...
#(
  ...
  logic [7:0]  obuf [0:OUT_BUF_BYTES-1];
  logic [10:0] wr_bi;
  logic [10:0] wr_beats;
  ...
  initial begin
    ...
    for (bi = 0; bi < OUT_BUF_BYTES; bi++) obuf[bi] = 8'h00;
  end
        ...
        S_RD_RECV: begin
          if (req_r_valid && r_ready_r) begin
            if (axis_r) begin
              ...
              // Each beat covers eight consecutive channels of one
              // (head, candidate) row: eight distinct accumulator slots.
              begin
                int b;
                int h2, c2, j2;
                b = int'(rd_bi) * 8;
                for (int j = 0; j < 8; j++) begin
                  ...
                  c2 = ((b + j) / 32) % int'(m_r);
                  j2 = (b + j) % 32;
                  acc[h2][j2] <= acc[h2][j2] + $signed(req_r_data[8*j +: 8]);
                end
              end
            end
          ...
          if (div_rsp_valid) begin
            begin
              ...
              if (!sign_r && rounded > 64'd127) res = 8'sd127;
                                ...
                                : $signed(rounded[7:0]);
              if (axis_r) row_res[div_idx[1:0]] <= res;
              else obuf[div_idx] <= res;
              if (axis_r && div_idx == 8'd2) begin
                // Row complete: commit the three head means into the
                // row-major dst slot without touching obuf[0:2] again.
                obuf[int'(cc_row) * 3 + 0] <= row_res[0];
                obuf[int'(cc_row) * 3 + 1] <= row_res[1];
                obuf[int'(cc_row) * 3 + 2] <= res;
              end
            end
            if (axis_r) begin
              if (div_idx == 8'd2) begin
