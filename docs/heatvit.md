...
| Q8 A-1 全量 30ep | 1.28M×30、lr 5e-5、fp32 | 未剪枝 77.26%（<Q2 起点，配方否决，见 §14.16） |
| Q9 A-2 全量 20ep | 1.28M×20、lr 2e-5、tf32、batch 256、5k spot 选优 | 未剪枝 **79.68%**（+1.82pp，距浮点 −0.54pp） |
| Q10 B-1/B-2 | A 系列主干 + 冻结 sup4 + λ=5 剪枝微调（修复后口径） | 剪枝 73.40% / **73.52%**（见 §14.16） |
| Q11 hw_w2 修复 | 双重转置缺陷定位与修复（daba669） | 旧剪枝口径数字作废（§14.16） |
...
| **A-2 QAT（全量 20ep）** | 位精确 · 5k | ✗ | **79.68%** | — |
| **B-1 λ=5（修复后口径）** | 位精确 · 5k | ✓ | **73.40%** | **92.0/42.6/35.6** |
| **B-2 λ=5（A-2 主干，修复后口径）** | 位精确 · 5k | ✓ | **73.52%** | 待回传 |

> **口径更正（2026-09-03）**：本表 59.12% / 68.20% / 74.00% / 73.94% /
> 72.70% / 67.76% 各行系 hw_w2 双重转置缺陷语义下测得（§14.16），保留
> 作历史记录但**已作废**；修复后口径以 §14.16 的 B-1/B-2 行为准，其余
> 旧行待重测后更正。未剪枝行不受影响。
...
**修复后口径更正（2026-09-03）**：本节 60.53% 与 102.2/50.6/42.4 系
hw_w2 双重转置缺陷语义下测得（见 §14.16），**作废待重测**；修复后
B-1 剪枝 5k 为 73.40%@92.0/42.6/35.6、B-2 为 73.52%，两者 50k 复核与
P4-2A 基线重测回传后更新本节表格。

### 14.16 全量 QAT（A-2）与部署换代（B-1/B-2，2026-09-03）

**hw_w2 双重转置缺陷（定位与修复，commit daba669）**：B-1 的 P5 导出前
`p5_crosscheck` 在 img2 上失败（stage-1 计数一致、Package token 第 105
列 1 LSB 分歧，下游翻转 4+6 个阈值边缘 token）。逐级探针（stage 隔离 +
hw 链重建 + 数值对拍）定位到 `p2_sim.token_selector` 的 `hw_acc` 对
hw_w2 多做了一次转置：`p2_train_selector` 存盘时已把 `Linear.weight`
转置存储，RTL、黄金模型、`QatSelector` 训练前向均直接消费 payload，
唯独模拟器再转一次。`git blame` 显示该行自 P2-C（2026-08-22，
14e4bef0）引入——此后所有**剪枝路径评估数字**（59.12%、68.20%、
74.00%、73.94%、72.70%、67.76%、50k 的 60.53% 及 §14.13 相关行）均在
错误语义下测得，**全部作废**，须用修复后管线重测；未剪枝数字与
XSim/RTL 侧不受影响（RTL 始终为黄金语义、XSim 向量由黄金模型生成，
P5 逐位回归一直有效）。修复后 test_qat 22 项全绿、p5_crosscheck 三图
bit-exact（img2 修复后 argmax 变化，证明缺陷影响预测）。

**A-1 全量 QAT（配方否决，2026-09-01）**：全量 1.28M × 30ep、lr 峰值
5e-5、fp32、batch 128：5k exact **77.26%**（e30），低于 Q2 起点
77.86%（−0.60pp）；训练中 e7「best」5k 仅 76.60%——高 lr 重启在长程上
把权重推向可微路径（e30 train-path 78.90%，两路径 gap 1.64pp），且
best 按 1000 窄子集选择失真。配方否决；教训：全量档峰值 lr 应降至
2e-5、best 按 5k spot 选（T3.1 `--spot-every`，commit ff0dbb4）。

**A-2 全量 QAT（2026-09-01→02）**：Q2 起步、lr 峰值 2e-5、tf32、
batch 256、20ep、`--spot-every 5`（每 5ep 5k 位精确抽查、best 按 5k）：
spot 5k exact **78.72% → 79.26% → 79.48% → 79.68%**（e20 终态），较 Q2
77.86% **+1.82pp**，距浮点同子集基线 80.22% 仅 **−0.54pp**——全项目
最大单次提升；低峰值 lr + tf32 + 5k spot 选优三修正全部兑现。

**B-1/B-2 剪枝微调与部署换代**：率正则 λ=5、冻结 sup4、32k×5
（`--spot-every 5` 末轮 5k 位精确剪枝验收；以下均为修复后口径）：

| 版本 | 主干 | 剪枝 5k | 计数（目标 88/45/32） | 备注 |
| --- | --- | ---: | --- | --- |
| B-1 | Q2（77.86%） | best **73.40%** / ckpt 73.32% | 92.0/42.6/35.6 | 旧口径 73.44/73.34，修复后几乎不变 |
| B-2 | A-2（79.68%） | **73.52%** | 待回传 | 主干 +1.82pp 仅传导 +0.12pp：瓶颈在冻结选择器排序质量（§14.11 结论重现） |

B-2 为当前部署候选；B-1 亦完成硬件闭环（下）。P4-2A 旧基线的修复后
重测（对照锚点）待补（本地或上传后 5k 级）。

**P5-B1/B2 导出与逐位回归（2026-09-01→03）**：B-1、B-2 依次导出回
RTL（`p5_export.ps1`：crosscheck → `--write-rom` → 每图向量）并跑
6 轮 XSim：

| 轮次 | B-1 周期 | B-2 周期 |
| --- | ---: | ---: |
| img0 无回压 | 273,664,425 | 251,702,691 |
| img0 回压 | 304,467,155 | 280,043,284 |
| img1 无回压 | 241,917,543 | 240,198,457 |
| img1 回压 | 269,141,643 | 267,242,863 |
| img2 无回压 | 233,550,672 | 220,304,409 |
| img2 回压 | 259,835,673 | 245,102,593 |

全部 TEST_PASS。B-2 周期全面低于 B-1（剪枝更激进：img0
197→128→71→57）。B-2 六轮运行中第三轮曾因机器睡眠被外部终止，
`.pass` 断点续跑零损失补救。两次闭环后均按第三部分 §9 步骤 3 恢复
合成状态（描述符/e2e 向量重新生成）。

**待办**：B-2 计数与剪枝 50k、A-2 未剪枝 50k、P4-2A 修复后基线重测；
结果回传后更新 §14.13/§14.15 表格与本项目 README。
