...
- **第一部分 设计规格**：目标与依据、模型数据流、顶层架构、描述符调度、
  ...
  黄金模型、验收标准
- **第二部分 实施计划与实施记录**：五个阶段的执行蓝图、任务清单与
  as-built 实施记录（含接口裁定、踩坑与验收证据）
...
`p2_train_selector.py`（训练 + 导出）、`p2_train_selector_float.py`
（浮点主干训练路径）、`p2_teacher_logits.py`（教师 logits 缓存）、
`p2_selector_eval.py`（精确仿真器剪枝评估）。
...
| 配置 | 计数（目标 88/45/32） | Top-1 @1k |
| --- | --- | --- |
| 未剪枝（无 Selector） | 197/197/197 | 84.7% |
| 速率引导训练（整数主干，无任务梯度） | 93.4/42.7/24.8 | **37.3%** |
| 浮点主干训练（有任务梯度，CE 0.86） | 2/2/2（全剪除） | 0.7% |

机制链（尺度校准 → 速率控制 → 阈值补偿）已端到端打通：计数可精确
命中目标。但精度未恢复：速率引导的选择器剪枝近似随机（−47pp）；浮点
主干训练的决策边界依赖浮点特征，转移到 int8 主干特征时整体下移
（全剪除）。**剩余差距的唯一通路是 STE 镜像 QAT**：在训练循环中用
假量化（fake-quant + straight-through）逐位镜像整数主干的非线性与
尺度，让任务梯度在「真实量化特征」上流动——即 HeatViT 论文
Block-to-Stage 训练的量化感知版本；或直接对主干+选择器联合 QAT
（超出纯 PTQ 口径，对应论文 71.9% 的训练管线）。
